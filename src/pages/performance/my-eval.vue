<template>
  <view class="my-eval-page" v-if="myPerformance">
    ...
    <SelfEvalForm
      ...
      :editable="isSelfEvaluating"
      @save="handleSaveSelfEval"
      @submit="handleSubmitSelfEval"
    />
...
const isSelfEvaluating = computed(() =>
...
function handleSubmitIndicators() {
  store.submitIndicatorsForConfirm()
  uni.showToast({ title: '已提交确认', icon: 'success' })
}

function handleSaveSelfEval({ content, score }) {
  store.submitSelfEval(content, score)
  uni.showToast({ title: '自评已保存', icon: 'success' })
}

function handleSubmitSelfEval({ content, score }) {
  store.submitSelfEval(content, score)
